...
import com.serviloc.mission.application.dto.request.RejectQuoteRequest;
...
public class ClientDemandController {
    ...
    @GetMapping("/demands/{id}/quotes")
    public ResponseEntity<ApiResponse<List<QuoteResponse>>> getQuotes(
            @PathVariable String id) {

        return ResponseEntity.ok(ApiResponse.success(demandUseCase.getQuotesForDemand(id)));
    }
