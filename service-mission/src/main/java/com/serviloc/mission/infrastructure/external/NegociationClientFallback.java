...
public class NegociationClientFallback implements NegociationClient {
    ...
    public List<QuoteDto> getQuotesByDemand(String demandId) {
        log.warn("NegociationClient indisponible — liste des devis non résolue pour demande={}", demandId);
        return Collections.emptyList();
    }

    @Override
    public QuoteDto getQuoteByDemandAndProvider(String demandId, String providerId) {
        log.error("NegociationClient indisponible — devis non résolu pour demande={} provider={}", demandId, providerId);
        throw new IllegalStateException("Service Négociations indisponible — impossible de récupérer le devis. Réessayez ultérieurement.");
    }
}